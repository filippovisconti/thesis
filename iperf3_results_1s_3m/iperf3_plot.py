...
class iperf3_plotter(object):
    ...
    def plotLine(self, dataset, filename, desc, title, figureN):
        ...
        for c in dataset.columns:
            ...
            if self.upperbond > 0:
                axs[i].plot(dataset.index, [upperbond] * len(dataset.index),
                            "--",
                            lw=2,
                            color="red",
                            alpha=0.5)
            if self.upperbond > 0:
                axs[i].plot(dataset.index, [lowerbond] * len(dataset.index),
                            "--",
                            lw=2,
                            color="red",
                            alpha=0.5)
    ...
    def plotLinePretty(self, dataset, filename, desc, title):
        ...
        y_level = int(
            (int(dataset.values.max()) - int(dataset.values.min())) / 20)
        if y_level == 0:
        ...
        plt.yticks(
            np.arange(y_min, y_max, y_level),
            [str(round(x, 3)) for x in np.arange(y_min, y_max, y_level)],
            fontsize=14)
        plt.xticks(fontsize=14)

        for y in np.arange(y_min, y_max, y_level):
            plt.plot(np.arange(x_min, x_max),
                     [y] * len(np.arange(x_min, x_max)),
                     "--",
                     lw=1,
                     color="black",
                     alpha=0.3)
        ...
        for (upper_bound, lower_bound, tags) in self.bound:
            ...
            if upper_bound > 0:
                plt.plot(dataset.index, [upper_bound] * len(dataset.index),
                         lw=2,
                         color=tableau20[j % 20])
            plt.text(x_max,
                     upper_bound,
                     tags,
                     fontsize=12,
                     color=tableau20[j % 20])
            if lower_bound > 0:
                plt.plot(dataset.index, [lower_bound] * len(dataset.index),
                         lw=2,
                         color=tableau20[j % 20])
            plt.text(x_max,
                     lower_bound,
...
class iperf3_dataParser(object):
    ...
    def parseOptions(self, args):
        ...
        if options.plotFiles == "":
            ...
            self.plotFiles = map(
                lambda x: x.strip(), options.plotFiles.split(",")
            ) if "," in options.plotFiles else [options.plotFiles.strip()]

        if options.noPlotFiles == "":
            self.noPlotFiles = []
        else:
            self.noPlotFiles = map(
                lambda x: x.strip(), options.noPlotFiles.split(",")
            ) if "," in options.noPlotFiles else [options.noPlotFiles.strip()]
        ...
        if options.bound == "":
            ...
            # print(options.bound)
            bounds_tmp = map(lambda x: x.strip(), options.bound.split(
                "]")) if "]" in options.bound else [options.bound.strip()]
        ...
        return (self.foldername, list(self.plotFiles), list(self.noPlotFiles),
                self.output, options.upperLimit, options.lowerLimit,
                self.bound)
        ...
        for i in iperf.get('intervals'):
            for ii in i.get('streams'):
                if (round(float(ii.get('start')), 0)) <= duration:
                    ...
                    value.append(
                        round(
                            float(ii.get('bits_per_second')) / (1024 * 1024),
                            3))
    ...
    def get_dataset(self, plotFiles):
        ...
        for name in plotFiles:
            ...
            try:
                ...
                print('Could not parse JSON from file (ex): {0}'.format(
                    str(ex)))
...
if __name__ == '__main__':
    ...
    s = iperf3_dataParser()
    (foldername, plotFiles, noPlotFiles, output, upperLimit, lowerLimit,
     bound) = s.parseOptions(sys.argv[1:])
    print(
        'foldername {}\nplotFiles {}\nnoPlotFiles {}\noutput {}\nupperLimit {}\nlowerLimit {}\nbound {}\n'
        .format(foldername, str(plotFiles), str(noPlotFiles), output,
                upperLimit, lowerLimit, str(bound)))
    ...
    if len(plotFiles) > 0:
        ...
        if upperLimit >= 0 and lowerLimit >= 0:
            ...
            while e < len(dataset.columns):
                ...
                output_single = output.replace(".png",
                                               "_line_" + str(s) + ".png")
        ...
        if len(bound) >= 0:
            dataset = dataset.reindex(dataset.mean().sort_values().index,
                                      axis=1)
